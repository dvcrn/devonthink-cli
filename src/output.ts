...
export function printSchemaHuman(input: {
  ...
  for (const [name, property] of Object.entries(properties)) {
    ...
    if (property.enum?.length) {
      console.log(`  choices: ${property.enum.map((item) => formatPrimitive(item)).join(", ")}`);
    }
  }
}
...
function printObject(value: Record<string, unknown>, indent: number): void {
  ...
  const complexEntries = entries.filter(([, entryValue]) => !isScalar(entryValue));
...
function flattenObject(value: Record<string, unknown>, prefix = ""): Record<string, unknown> {
  ...
  for (const [key, entryValue] of Object.entries(value)) {
    ...
    if (entryValue && typeof entryValue === "object") {
      Object.assign(result, flattenObject(entryValue as Record<string, unknown>, nextKey));
      continue;
    }
...
function describeSchemaType(schema: JsonSchema): string {
  ...
  if (schema.type === "array") {
    return schema.items ? `array<${describeSchemaType(schema.items)}>` : "array";
  }
