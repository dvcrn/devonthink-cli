...
function lookupById(theApp, id, database) {
  if (id === null || id === undefined || typeof id !== "number") return null;
  try {
    if (database) {
      return theApp.getRecordWithId(id, { in: database });
    }
...
function getRecord(theApp, options) {
  ...
  if (options.id !== null && options.id !== undefined) {
    record = lookupById(theApp, options.id, options.database);
    if (record) {
      const result = {};
      result["record"] = record;
      result["method"] = "id";
