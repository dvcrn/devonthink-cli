...
function registerToolCommand(cli: Argv, tool: Tool): Argv {
  ...
  return cli.command(
    ...
    (yargsInstance) => configureToolCommand(yargsInstance, tool, positionalProperty),
    async (argv) => {
...
function configureToolCommand(
  ...
  if (positionalProperty) {
    ...
    if (propertySchema) {
      configured = configured.positional(toKebabCase(positionalProperty), {
        describe: propertySchema.description ?? `Value for ${positionalProperty}`,
  ...
  for (const [propertyName, propertySchema] of Object.entries(properties)) {
    configured = configured.option(toKebabCase(propertyName), {
      alias: propertyName === toKebabCase(propertyName) ? undefined : propertyName,
      array: isArraySchema(propertySchema) && isPrimitiveSchema(propertySchema.items),
      ...
      demandOption: required.has(propertyName) && propertyName !== positionalProperty,
...
function buildToolInput(tool: Tool, argv: Record<string, unknown>): Record<string, unknown> {
  const schema = tool.inputSchema as JsonSchema;
  const properties = schema.properties ?? {};
...
function coerceOptionValue(schema: JsonSchema, value: unknown): unknown {
  ...
  if (Array.isArray(value) && isArraySchema(schema) && !isPrimitiveSchema(schema.items)) {
